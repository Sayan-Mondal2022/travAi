...
const TRAVEL_TYPES = [
  ...
  { id: 'couple', label: 'Couple', emoji: '👩‍❤️‍👨', description: 'Romantic getaway' },
  { id: 'family', label: 'Family', emoji: '👨‍👩‍👧‍👦', description: 'With family members' },
  { id: 'friends', label: 'Friends', emoji: '👯‍♂️', description: 'Group of friends' },
  { id: 'business', label: 'Business', emoji: '💼', description: 'Business trip' },
];
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4">
            <button
              type="submit"
              className="w-full bg-purple-600 text-white py-3 px-6 rounded-xl hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 font-semibold shadow-lg transform hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div className="mt-6 p-3 bg-gray-100 rounded-lg">
          <p className="text-sm text-gray-600 text-center">
            ✈️ From <span className="font-semibold">{JSON.parse(localStorage.getItem('tripData') || '{}').from_location || '...'}</span>{' '}
            to <span className="font-semibold">{JSON.parse(localStorage.getItem('tripData') || '{}').to_location || '...'}</span>
          </p>
        </div>
      </div>
    </div>
